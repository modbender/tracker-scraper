...
def scrape_udp(parsed_tracker, hashes):
    ...
    if len(hashes) > 74:
        raise RuntimeError(
            "Only 74 hashes can be scraped on a UDP tracker due to UDP limitations")
...
def udp_parse_connection_response(buf, sent_transaction_id):
    ...
    res_transaction_id = struct.unpack_from(
        "!i", buf, 4)[0]  # next 4 bytes is transaction id
...
def udp_create_scrape_request(connection_id, hashes):
    ...
    buf = struct.pack("!q", connection_id)  # first 8 bytes is connection id
    buf += struct.pack("!i", action)  # next 4 bytes is action
    # followed by 4 byte transaction id
...
def udp_parse_scrape_response(buf, sent_transaction_id, hashes):
    if len(buf) < 16:
        raise RuntimeError(
            "Wrong response length while scraping: %s" % len(buf))
    action = struct.unpack_from("!i", buf)[0]  # first 4 bytes is action
    res_transaction_id = struct.unpack_from(
        "!i", buf, 4)[0]  # next 4 bytes is transaction id
    if res_transaction_id != sent_transaction_id:
    ...
    if action == 0x2:
        ...
        for hash in hashes:
            ...
            offset += 4
            ret[hash] = {"seeds": seeds,
                         "peers": leeches, "complete": complete}
        return ret
    elif action == 0x3:
